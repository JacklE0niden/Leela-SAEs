...
def run_batch_generation(*, folders: list[Path], n: int) -> None:
    print(f"Running batch generation with top-{n} features")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate feature intervention CSVs from attribution JSON files."
    )
    parser.add_argument(
        "--folder",
        dest="folders",
        ...
        required=True,
        type=Path,
        help="Folder containing infl_all_feature.json (repeat for multiple folders).",
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=TOP_N_PER_MOVE,
        help=f"Maximum number of features per move (default: {TOP_N_PER_MOVE}).",
    )
...
def main() -> None:
    args = parse_args()
    run_batch_generation(folders=args.folders, n=args.top_n)
