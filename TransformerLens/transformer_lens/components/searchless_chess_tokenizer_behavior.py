...
class SearchlessChessBehavioralCloningTokenizer(nn.Module):
    """
    ...
    def forward(
        ...
        Returns:
            sequences: [batch_size, 78] 的 torch 张量（77个FEN token + 1个dummy_actions）
        ...
        for single_input in input:
            sequences, fen = self._forward_single(single_input)
    ...
    def _forward_single(
        ...
        sequences = np.expand_dims(tokenized_fen, axis=0)
        
        sequences = np.concatenate([sequences, dummy_actions], axis=1)
        sequences = torch.tensor(sequences)
        # sequences = sequences.reshape(1, -1)  # [1, 78]
    ...
    def decode(self, tokens: np.ndarray) -> str:
        ...
        assert len(tokens) == 78, f"Expected 78 tokens, got {len(tokens)}"
        
        # 分离FEN tokens、action token和dummy token
        fen_tokens = tokens[:77]  # 前77个是FEN tokens
        dummy_actions = tokens[77]  # 第78个是dummy_actions
        
        # 解码FEN部分
        fen_str = self._decode_fen_tokens(fen_tokens)
        
        # 解码action部分
        if int(dummy_actions) in self.action_to_move:
            move_str = self.action_to_move[int(dummy_actions)]
        else:
            move_str = f"<unknown_action_{dummy_actions}>"
        
        # 解码dummy部分
        # dummy_token通常为0，这里直接转为字符串
        action_str   = str(int(dummy_actions))
        
        # 组合结果：直接拼接，不加任何分隔符
        result = f"{fen_str}{action_str}"
        return result
