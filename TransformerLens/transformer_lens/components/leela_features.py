...
class LeelaBoard:
    ...
    def serialize_features(self):
        ...
        def bytes_iter():
            ...
            yield np.packbits((curdata.us_ooo, curdata.us_oo, curdata.them_ooo, curdata.them_oo, curdata.side_to_move)).tobytes()
    ...
    def deserialize_features(cls, serialized):
        planes_stack = []
        rule50_count = serialized[-1]
        board_attrs = np.unpackbits(memoryview(serialized[-2:-1]))
        ...
        for history_idx in range(0, bytes_per_history * 8, bytes_per_history):
            ...
            if not side_to_move:
                planes = np.unpackbits(memoryview(plane_bytes))[::-1].reshape(12, 8, 8)[::-1]
            else:
                planes = (
    ...
    def _uci_to_idx_dict(self):
        ...
        return uci_to_idx[uci_to_idx_index]

    def _idx_to_uci_dict(self):
        data = self.lcz_stack[-1]
        uci_to_idx_index = (data.us_ooo | data.us_oo) + 2 * data.side_to_move
        return idx_to_uci[uci_to_idx_index]
    ...
    def _repr_svg_(self):
        return chess.svg.board(
            ...
            check=self.pc_board.king(self.pc_board.turn) if self.pc_board.is_check() else None,
            colors={
                "square light": "#f5f5f5",
                "square dark": "#cfcfcf",
    ...
    def __str__(self):
        if self.pc_board.is_game_over() or self.is_draw():
            ...
            turnstring = f"Turn: {'White' if self.pc_board.turn else 'Black'}"
    ...
    def __call__(self, input=None, no_history=False, history_synthesis=True, moves=None, uci=False):
        """
        Support input of single FEN string or list of FEN strings.
        If list, return tokens with first dimension as batch size.
        """
        if isinstance(input, (list, tuple)):
            tokens_list = [
                self.__call__(fen, no_history=no_history, history_synthesis=history_synthesis, moves=moves, uci=uci)
                for fen in input
            ]
            return torch.cat(tokens_list, dim=0)  # or use torch.stack
        elif isinstance(input, str):
            board = self.from_fen(input, moves=moves, uci=uci, history_synthesis=True)
            features = board.lcz_features(no_history=False)
            tokens = torch.from_numpy(features).float().unsqueeze(0)
            return tokens
        else:
