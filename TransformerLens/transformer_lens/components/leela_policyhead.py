...
class Mish(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        weight = self.hook_weight(weight) # policy_head.mish.hook_weight
        return x * weight
...
class PolicyHead(nn.Module):
    ...
    def __init__(self, d_model: int = 768, policy_dim: int = 1858):
        ...
        # self.promotion_weight = nn.Parameter(torch.randn(3))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print(f"{promotion_slice2.shape = }")
        promotion_out2 = promotion_slice2.reshape(-1, 64, 1)
        # print(f"{promotion_out2.shape = }")
        ...
        # print(f"{promotion.shape = }")
        
        policy = torch.cat([scores, promotion], dim=1)
        # print(f"{policy.shape = }") [1, 67, 64]
    
        policy = policy.reshape(-1, 4288)
        # print(f"{policy.shape = }")
        
        indices_long = self.indices.detach().long()
        # torch.set_printoptions(profile="full")     # 取消截断
        # print(f"{indices_long = }")                # 或 print(indices_long)
        # torch.set_printoptions(profile="default")  # 打印完恢复默认（可选）
