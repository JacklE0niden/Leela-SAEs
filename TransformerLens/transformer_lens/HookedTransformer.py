...
class HookedTransformer(HookedRootModule):
    ...
    def __init__(
        ...
        if tokenizer is not None:
            ...
            if self.cfg.tokenizer_name in NON_HF_HOSTED_MODEL_NAMES:
                ...
                elif tokenizer is not None and self.cfg.is_leela_chess_model is True:
        ...
        if self.cfg.is_leela_chess_model:
            ...
            elif self.cfg.leela_embed == "advanced":
                print("advanced embedding for BT4")
                self.embed = BT4LeelaEmbed(self.cfg.d_model)
        ...
        if self.cfg.is_leela_chess_model:
            ...
            self.mlh_head = MLHHead(self.cfg.d_model, self.cfg.d_mlh_head)
    ...
    def input_to_embed(
        ...
        if isinstance(tokens, torch.Tensor) and len(tokens.shape) == 1:
            ...
            tokens = tokens[None]
        if isinstance(tokens, torch.Tensor) and tokens.device.type != self.cfg.device:
        ...
        with utils.LocallyOverridenDefaults(
            ...
            if self.cfg.is_leela_chess_model:
                if stop_at_layer is not None:
                    return residual
                policy_logits = self.policy_head(residual)
                value_logits = self.value_head(residual)
                mlh_logits = self.mlh_head(residual)
    ...
    def move_model_modules_to_device(self):
        ...
        if self.cfg.is_leela_chess_model:
            ...
            self.mlh_head.to(devices.get_device_for_block_index(0, self.cfg))
