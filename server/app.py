...
app.include_router(
    get_circuit_taxonomy_router(
        client=client,
        sae_series=sae_series,
        repo_root=REPO_ROOT,
        roots=CIRCUIT_TAXONOMY_ROOTS,
        get_dataset=get_dataset,
        get_hooked_model=get_hooked_model,
...
def _preload_default_sae_combo_on_startup() -> None:
    if os.environ.get("PRELOAD_CIRCUIT_MODELS", "false").lower() not in {
        "1",
        "true",
        "yes",
    }:
        print("Skipping default SAE combo preload (set PRELOAD_CIRCUIT_MODELS=true to enable)")
        return
